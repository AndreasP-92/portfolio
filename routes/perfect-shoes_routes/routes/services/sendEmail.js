    ...
    order: function (mail, products, totalPrice) {
        ...
        let mailOptions = {
            ...
            attachments: [{
                ...
                path: './routes/perfect-shoes_routes/template/images/thankyou.jpeg',
...
// ******************************************************************************* SEND RESET PASS *******************************************************************'
    ...
    resetPass: function(token, done, req){
        ...
        let mailOptions = {
            ...
            text: `You're recieving this email, because you've asked to reset your password. Please follow the link http://${req.headers.host}/perfect-shoes/reset/${token} the password.`
        };
        let dato = new Date()
        smtpTransport.sendMail(mailOptions, (error, info) => {
            if (error) {
